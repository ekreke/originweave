...
- **M1 · 黑板与 Agent 循环**（见 `SPEC.md` 的 M1 小节）。样例的录制 query/prompt 清单见
  `examples/copilot_productivity/README.md`，编排实现需与其对齐。
...
- [x] run dir 的默认根目录 → `trace` 写 `runs/`（gitignored，`Makefile` 的 `RUNS_DIR`），
  `replay`/`ui` 读已入库的样例目录（`RUN_DIR`）；两者不再混用（M0d 定）
...
- `frontend/`（仅有构建产物，源码从未入库）已在清理中整体移除；M4 需**从零重建前端源码工程**。
  在此之前 UI 不能视为可维护资产。
- `Makefile` 的 `demo` target 依赖尚不存在的 `trace` 实现，现阶段执行会命中 stub（M1/M2 落地）。
